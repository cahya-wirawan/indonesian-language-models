...
LANG_FILENAMES = [DATA_PATH/'valid1000']
...
LANG_TEXT.to_csv("{}/emLam.csv".format(LM_PATH), index=False)
LANG_TEXT = pd.read_csv("{}/emLam.csv".format(LM_PATH))
#LANG_TEXT.assign(labels = 0)
LANG_TEXT.columns = ['text']
LANG_TEXT.insert(loc=0, column='labels', value=0)
LANG_TEXT.to_csv("{}/emLam.csv".format(LM_PATH), header=None, index=False)
#(LANG_TEXT.assign(labels = 0).pipe(lambda x: x[['labels', 'text']])
# .to_csv("{}/emLam.csv".format(LM_PATH), header=None, index=False))
...
LANG_TEXT = pd.read_csv("{}/emLam.csv".format(LM_PATH), header=None)#, chunksize=5000)
